...
class Noah3(Agent):
    ...
    def new_game(self, number_of_players, player_number, spies):
        ...
        print(f"(agent {self.id}) NEW GAME WITH {number_of_players} PLAYERS, {s} SPIES, AND {spies}")

        if self.spy:
            self.possible_worlds = {world: 1 for world in combinations(self.players, s)} #changed this
        else:
    ...
    def propose_mission(self, team_size, betrayals_required):
        ...
        if self.spy:
            ...
            choice = [self.id] + spies[betrayals_required-1:] + non_spies[team_size - len(spies)+ 1:] # is this right?
        else:
